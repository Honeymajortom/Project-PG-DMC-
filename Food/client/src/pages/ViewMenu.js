        ...
        const searchMenu = () => {
          axios.get(`http://localhost:4000/menu`).then((response) => {
            ...
            if (result["status"] === "success") {
              ...
              const { id, title, descr, thumbnail } = result["data"][0];
              console.log(response.data)

              // persist the logged in user's information for future use
              sessionStorage["id"] = id;
              sessionStorage["title"] = title;
              sessionStorage["descr"] = descr;
                ...
                {menus.map((item)=>{
                  return(
                    <Card style={{ width: '18rem' }}>
                      <Card.Img variant="top" src={'http://localhost:4000/' + item.thumbnail} alt="" className="thumbnail-sm" />
                      <Card.Body>
